...
jobs:
  precommit:
    ...
    secrets: inherit
    with:
      atomi_platform: let__platform__
      atomi_service: let__service__
```
...
Cache keys MUST use LPSM naming: `let__platform__-let__service__-*`

```yaml
nscloud-cache-tag-let__platform__-let__service__-nix-store-cache
```
...
| **Cache namespacing**     | `let__platform__-let__service__-nix-store-cache`       |
