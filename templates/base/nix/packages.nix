  ...
  all = rec {
    ...
    nix-2605 = (
      ...
      rec {
        ...
        # Keep the assignment inside the resolver-supported registry block so
        # compositions cannot replace it with the channel's unrelated `nsc`.
        nsc = (
          let
          ...
          pkgs-2605.stdenv.mkDerivation {
            ...
            '';
            setupHook = pkgs-2605.writeText "diene-nsc-setup-hook" ''
              export DIENE_NSC_BIN=@out@/bin/nsc
              export DIENE_NSC_IDENTITY_FILE=@out@/share/diene/nsc-identity.json
            '';
