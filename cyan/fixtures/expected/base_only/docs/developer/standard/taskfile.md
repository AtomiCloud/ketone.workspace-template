...
Taskfiles should delegate to shell scripts or real commands. Avoid echo statements for progress feedback — the task runner automatically displays the task being executed and its commands. Shell scripts should have echo statements for progress. Echo in Taskfiles is acceptable only for minimal stub/placeholder commands (e.g., `echo "Completed"` in a base template).
...
| **Echo statements** | Avoid in Taskfile (stubs OK), YES in shell scripts |
